...
**片方向遅延の限界**: Fwd/Bwd が真の片方向遅延を表すのは、Sender と Reflector のクロックが同期している場合に限られる。非同期クロックでは、単一往復の 4 タイムスタンプ (T1..T4) から「経路の非対称性」と「クロックオフセット」を分離できない（未知数 `d_f, d_b, θ` の 3 個に対し独立な式が 2 本のみ＝劣決定）。Offset を使った「補正」（`Fwd − Offset`, `Bwd + Offset`）は経路対称（`d_f = d_b`）を暗黙に仮定することと等価で、結果は常に `RTT/2` に潰れ追加情報を持たないため、補正カラムは提供していない。正確な片方向遅延には PTP/PHC 等による両端クロック同期が必要で、同期下では生の Fwd/Bwd がそのまま真の片方向遅延になる（Offset は残留同期誤差の診断値）。
